...
execute if score vaults_from_ominous_raid_captains skyvoid_additions matches 1 at @a[tag=skyvoid_additions_started_raid] as @e[tag=!skyvoid_additions_replaced_bell,distance=..30,type=#minecraft:illager,predicate=skyvoid_additions:ominous_raid_captains/captain_riding_ravager] on vehicle at @s run function skyvoid_additions:mechanics/ominous_raid_captains/find_bell
